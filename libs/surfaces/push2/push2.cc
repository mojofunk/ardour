...
Push2::set_state (const XMLNode & node, int version)
{
	...
	if (!node.get_property (X_("root-octave"), _root_octave)) {
		node.get_property (X_("root_octave"), _root_octave);
	}

	if (!node.get_property (X_("in-key"), _in_key)) {
		node.get_property (X_("in_key"), _in_key);
	}
