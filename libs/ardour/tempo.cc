...
MetricSection::add_state_to_node(XMLNode& node) const
{
	...
	node.set_property ("frame", _frame);
	node.set_property ("movable", _movable);
...
MetricSection::set_state (const XMLNode& node, int /*version*/)
{
	...
	node.get_property ("frame", _frame);

	if (!node.get_property ("movable", _movable)) {
	...
	, _locked_to_meter (false)
{
	...
	if (!node.get_property ("beats-per-minute", _beats_per_minute)) {
		error << _("TempoSection XML node has no \"beats-per-minute\" property") << endmsg;
		throw failed_constructor();
	}

	if (_beats_per_minute < 0.0) {
...
TempoSection::get_state() const
{
	...
	root->set_property ("beats-per-minute", _beats_per_minute);
