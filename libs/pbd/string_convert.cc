...
bool string_to_double (const std::string& str, double& val)
{
	...
	if (errno == ERANGE) {
		DEBUG_SCONVERT (
		    string_compose ("string_to_double possible conversion failure for %1", str));
...
bool int16_to_string (int16_t val, std::string& str)
{
	...
	if (retval <= 0 || retval >= (int)sizeof(buffer)) {
		DEBUG_SCONVERT (
		    string_compose ("int16_to_string conversion failure for %1", str));
		return false;
	}
	str = buffer;
	return true;
...
bool uint16_to_string (uint16_t val, std::string& str)
{
	...
	if (retval <= 0 || retval >= (int)sizeof(buffer)) {
		DEBUG_SCONVERT (
		    string_compose ("uint16_to_string conversion failure for %1", str));
		return false;
	}
	str = buffer;
	return true;
...
bool int32_to_string (int32_t val, std::string& str)
{
	...
	if (retval <= 0 || retval >= (int)sizeof(buffer)) {
		DEBUG_SCONVERT (
		    string_compose ("int32_to_string conversion failure for %1", str));
		return false;
	}
	str = buffer;
	return true;
...
bool uint32_to_string (uint32_t val, std::string& str)
{
	...
	if (retval <= 0 || retval >= (int)sizeof(buffer)) {
		DEBUG_SCONVERT (
		    string_compose ("uint32_to_string conversion failure for %1", str));
		return false;
	}
	str = buffer;
	return true;
...
bool int64_to_string (int64_t val, std::string& str)
{
	...
	if (retval <= 0 || retval >= (int)sizeof(buffer)) {
		DEBUG_SCONVERT (
		    string_compose ("int64_to_string conversion failure for %1", str));
		return false;
	}
	str = buffer;
	return true;
...
bool uint64_to_string (uint64_t val, std::string& str)
{
	...
	if (retval <= 0 || retval >= (int)sizeof(buffer)) {
		DEBUG_SCONVERT (
		    string_compose ("uint64_to_string conversion failure for %1", str));
		return false;
	}
	str = buffer;
	return true;
...
bool double_to_string (double val, std::string& str)
{
	...
	if (d_cstr == NULL) {
		DEBUG_SCONVERT (
		    string_compose ("double_to_string conversion failure for %1", val));
		return false;
	}
	str = d_cstr;
	return true;
